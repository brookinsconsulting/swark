...
class SwarkShortenWOperator extends SwarkOperator
{
    ...
    static function execute( $operatorValue, $namedParameters )
    {
        ...
        if ( $length && strlen( $operatorValue ) > $length )
        {
            $shortened = substr( $operatorValue, 0, $length - $ellipsisLength );
            if ( $operatorValue[$length - $ellipsisLength] != ' ' )
            {
                $pos = strrpos( $shortened, ' ' );
                if ( $pos !== false )
                {
                    $shortened = substr( $shortened, 0, $pos );
                }
            }
