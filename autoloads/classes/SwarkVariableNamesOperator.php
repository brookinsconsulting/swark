...
class SwarkVariableNamesOperator extends SwarkOperator
{
                     ...
                     $currentNamespace, &$operatorValue, $namedParameters )
    {
        ...
        foreach ( $tpl->Variables as $nameSpace => $variables )
        {
            ...
            {
                $variableNames[] = '$' . ( $nameSpace? "$nameSpace:": '' ) . $variableName;
            }
        }

        $variableNamesText = implode( $variableNames, ', ' );
        if ( $namedParameters['in_debug'] )
        {
            eZDebug::writeNotice( $variableNamesText, 'Variable names operator' );
        }
        else
        {
            $operatorValue = $variableNamesText;
        }
    }                     
